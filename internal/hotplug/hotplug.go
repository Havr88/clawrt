...
func ProcessHotplugEvent(cfg *config.Config, args []string) error {
	...
	case "iface", "network":
		...
		if action == "ifdown" {
			alertMsg = fmt.Sprintf("🚨 *Alerta de Red (WAN/Interfaz):*\nLa interfaz `%s` se ha **DESCONECTADO**.\n\n🛠️ *Iniciando protocolo de Auto-Sanación autónomo en segundo plano...*", ifaceName)
			// Trigger autonomous watchdog healing asynchronously
			go func() {
				...
				if diag.OverallStatus != watchdog.StatusHealthy {
					...
					if recovered {
						healMsg := fmt.Sprintf("✅ *Auto-Sanación Completada con Éxito:*\nInterfaz `%s` restablecida automáticamente.\n📍 Acciones: %s", ifaceName, strings.Join(actions, " ➡️ "))
						for _, cid := range cfg.ChatIDs {
							_ = bot.SendMessage(cid, healMsg)
						}
					}
				}
			}()
