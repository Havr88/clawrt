...
const (
	TierExtremeMinimal HardwareTier = "extreme_minimal" // 4MB Flash / 32MB RAM (Legacy Hardware: TP-Link WR741ND / OpenWrt 18.06/19.07)
	TierMinimal        HardwareTier = "minimal"         // 8MB Flash / 64MB RAM (Bare Minimum: Xiaomi Mi Router 4C / mipsel_24kc)
	TierMedium         HardwareTier = "medium"          // 16MB Flash / 128MB RAM (Recommended Standard: OpenWrt stable)
	TierFull           HardwareTier = "full"            // 32MB+ Flash / >256MB RAM (Enterprise / Linksys Velop / x86_64)
)
...
func (k *KnowledgeEngine) ClearFacts() {
	k.mu.Lock()
	defer k.mu.Unlock()
	k.MutableFacts = make(map[string]string)
	_ = os.Remove(k.storagePath)
}
